class IngredientesController < ApplicationController
  ...
  def index
    @q = Ingrediente.ransack(params[:q])
    @ingredientes = @q.result(distinct: true).paginate(page: params[:page], per_page: 2)
  end
  ...
  def destroy
    @ingrediente.destroy
    respond_to do |format|
      format.html { redirect_to ingredientes_url, notice: "Ingrediente excluído com sucesso." }
